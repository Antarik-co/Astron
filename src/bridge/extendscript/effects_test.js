...
(function () {
    ...
    Astron.utils.effects.addEffect = function (layer, effectMatchName) {
        return layer.property("ADBE Effect Parade").addProperty(effectMatchName);
    };
    ...
    Astron.utils.effects.removeEffect = function (layer, effectName) {
        var i;
        for (i = 1; i <= layer.property("ADBE Effect Parade").numProperties; i++) {
            if (layer.property("ADBE Effect Parade").property(i).name === effectName) {
                layer.property("ADBE Effect Parade").property(i).remove();
                return true;
            }
        }
        return false;
    ...
    Astron.utils.effects.getEffect = function (layer, effectName) {
        ...
        for (i = 1; i <= layer.property("ADBE Effect Parade").numProperties; i++) {
            if (layer.property("ADBE Effect Parade").property(i).name === effectName) {
                return layer.property("ADBE Effect Parade").property(i);
            }
        }
        return null;
    };
    ...
    //        - Required by layer.property("ADBE Effect Parade").addProperty()
    //
